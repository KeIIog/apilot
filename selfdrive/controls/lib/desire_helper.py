  ...
  def update_(self, carstate, lateral_active, lane_change_prob):
    ...
    if not lateral_active or self.lane_change_timer > LANE_CHANGE_TIME_MAX:
      ...
      # LaneChangeState.laneChangeStarting
        ...
        # fade out over .5s
        self.lane_change_ll_prob = max(self.lane_change_ll_prob - 2 * DT_MDL, 0.0) #2*DT_MDL#keiiog test
  ...
  def nav_update(self, carstate, roadLimitSpeed, road_edge_stat):
    ...
    if 5 < nav_distance < 3000 and direction != LaneChangeDirection.none:
      if self.desireReady >= 0: # -1이면 현재의 네비정보는 사용안함.
        ...
        elif nav_distance < 1000: # and self.navActive == 0: # 차로변경시작
          need_torque = 2
          nav_direction = direction
    ...
    if not lateral_active or self.lane_change_timer > LANE_CHANGE_TIME_MAX:
      ...
      # LaneChangeState.laneChangeFinishing
        ...
        if self.lane_change_ll_prob > 0.99:
          ...
          if one_blinker:
            self.lane_change_state = LaneChangeState.preLaneChange
            self.needTorque = False#True # 두번째부터 토크...
          else:
            self.lane_change_state = LaneChangeState.off
            self.lane_change_direction = LaneChangeDirection.none
